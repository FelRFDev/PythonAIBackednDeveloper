...
from relatorios import registros_transacoes, ContasRegistradas
        ...
        if (not self.nome_saque_entry.get() or not self.conta_saque_entry.get() or not self.senha_saque_entry.get()
            ...
            if check_box_valido:
                ...
                if conta_saque.saldo == 0:
                    ...
                    registro_gerador_saque = registros_transacoes(conta_saque, 'Saque')
                    for registro in registro_gerador_saque:
                        print(registro)
                ...
                case 'Pessoa Física':
                    try:
                        ...
                        try:
                            ...
                            gerador_deposito_pf = registros_transacoes(conta_utilizada_dep, 'Depósito')
                            for registro in gerador_deposito_pf:
                                print(registro)
                ...
                case 'Pessoa Jurídica':
                    try:
                        ...
                        try:
                            ...
                            # =-=-=-=-= Prints para verificação de informação =-=-=-=-=-
                            gerador_deposito_pj = registros_transacoes(conta_utilizada_dep, 'Depósito')
                            for registro in gerador_deposito_pj:
                                print(registro)
                            print(f'Total de transacoes realizadas {len(conta_utilizada_dep.historico.registros)}')
            ...
            match dados_validos:
                case True:
                    if secao == 'Pessoa Física':
                        ...
                        gerador_saldo_pf = registros_transacoes(conta_utilizada)
                        for registro in gerador_saldo_pf:
                            print(registro)
                        ...
                        gerador_saldo_pj = registros_transacoes(conta_utilizada)
                        for registro in gerador_saldo_pj:
                            print(registro)
                        saldo = str(conta_utilizada.exibe_saldo_tela())
                        self.tela_saldo(saldo)
...
if __name__ == '__main__':
    ...
    # -=-=-=-=-=-=-=-
    print('\nPrint do Iterador Clientes Pessoa Física\n')
    for cliente_pf_registrado in clientes_pf:
        for numero, saldo, agencia in ContasRegistradas(cliente_pf_registrado.contas):
            print(f'Número da conta: {numero} / Saldo da Conta {saldo} / Agência da conta: {agencia}\n')
    

    print('\nPrint do Iterador Clientes Pessoa Jurídica\n')

    for cliente_pj_registrado in clientes_pj:
        for numero, saldo, agencia in ContasRegistradas(cliente_pj_registrado.contas):
            print(f'Número da conta: {numero} / Saldo da Conta {saldo} / Agência da conta: {agencia}\n')